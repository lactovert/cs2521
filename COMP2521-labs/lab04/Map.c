...
int connections(Map g, LocationID start, LocationID end, TransportID type[]) {
    ...
    int i = 0;
    // if it is a port city then need to move to the sea itself before moving to other place
    for (VList curr = g->connections[start]; curr!=NULL; curr = curr->next) {
        if (curr->type == BOAT) {
            for (VList curr1 = g->connections[end]; curr1!=NULL; curr1 = curr1->next) {
                if (curr1->type == BOAT && curr1->v == curr->v) {
                    type[i]=curr1->type;
                    i++;
                }
            }
