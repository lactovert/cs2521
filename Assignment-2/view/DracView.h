...
// newDracView() creates a new game view to summarise the current
// state of the game.
//
...
// disposeDracView() frees all memory previously allocated for the
// DracView toBeDeleted, which should not be accessed after the call.
...
void lastMove (
    DracView dv, PlayerID player, LocationID *start, LocationID *end);
...
void whatsThere (
    DracView dv, LocationID where, int *numTraps, int *numVamps);
...
void giveMeTheTrail (
    DracView dv, PlayerID player, LocationID trail[TRAIL_SIZE]);
...
LocationID *whereCanIgo (
    DracView dv, int *numLocations, bool road, bool sea);
...
LocationID *whereCanTheyGo (
    DracView dv, int *numLocations,
    PlayerID player, bool road, bool rail, bool sea);

#endif // !defined(DRAC_VIEW_H_)
