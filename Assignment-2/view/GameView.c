...
typedef struct _player {
    LocationID v;
    LocationID history[MAX_TRAIL_SIZE];
    int health;
} player;
...
newGameView (char *pastPlays, PlayerMessage messages[])
{
    ...
    for(int i = PLAYER_LORD_GODALMING; i < NUM_PLAYERS; i++) {
        ...
        for(int j = 0; j < MAX_TRAIL_SIZE; j++) {
                    ...
                    if (id == NOWHERE) {
                        switch(abbrev[0]) {
                            ...
                            case 'D' :
                                ...
                                break;
                            case 'T' :
                                shiftTrail(TELEPORT, new->players[PLAYER_DRACULA].history);
                                new->players[PLAYER_DRACULA].v = TELEPORT;
                                break;
                        }
                    }  else {
    ...
    bool road, bool rail, bool sea)
{
    ...
    if(isUnknownLoc(from)) {
        from = deduceLoc(gv->players[player].history, ZERO);
    }
    *numLocations = connections(map, from, 
            player, round, road, rail, sea);
    LocationID *conn = malloc(sizeof(LocationID)*(*numLocations));
    for (int i = 0; i < (*numLocations); i++) {
        conn[i] = NOWHERE;
    }
    connectionsArray(map, from, conn, player, round, road, rail, sea);
    disposeMap(map);
    return conn;
}
...
static void shiftTrail(LocationID new, LocationID trail[]) {
    for (int n = MAX_TRAIL_SIZE-2; n >= 0; n--) {
        trail[n+1] = trail[n];
    }
...
static void update (LocationID id, char *abbrev, char enc[], GameView gv, PlayerID player) {
    ...
    if (player == PLAYER_DRACULA){
        ...
        // revive
        if (getLocation(gv, player) == ST_JOSEPH_AND_ST_MARYS  && getHealth(gv, player) <= ZERO) {
            ...
            if (gv->players[player].health < GAME_START_HUNTER_LIFE_POINTS) {
