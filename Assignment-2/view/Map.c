...
static int alreadyInTrail (LocationID curr, LocationID trail[], int index);
...
int connections(Map g, LocationID start, PlayerID player, Round round, bool road, bool rail, bool sea) {
    assert(g != NULL);
    int i = 0;
    if(isUnknownLoc(start)) {
        return 0;
    }
    LocationID conn[NUM_MAP_LOCATIONS];
    for(int i = 0; i < NUM_MAP_LOCATIONS;i++) conn[i]=NOWHERE;
    conn[i++] = start;
    // check if start is valid loc (not a doubleback)
...
static void bfs(Map map, LocationID from, int hops, LocationID conn[], int i) {
    ...
    while (!QueueIsEmpty(q)) {
        from = QueueLeave(q);
        hops = QueueLeave(hop);
        // don't put in conn[] if path length is wrong or that location
        // is already in the conn[]
        if (hops == 0 || alreadyInTrail(from, conn, i))
            continue;
        conn[i++] = from;
