...
static LocationID revealLoc (GameView gv, LocationID trail[], int i);
...
whereIs (DracView dv, PlayerID player)
{
    if (player == PLAYER_DRACULA) {
        ...
        getHistory(dv->gv, PLAYER_DRACULA, history);
        return revealLoc(dv->gv, history, ZERO);
    }
...
static LocationID revealLoc (GameView gv, LocationID trail[], int i) {
    ...
    if (isUnknownLoc(trail[i])) {
            switch (loc) {
                case HIDE :
                    if (!(isUnknownLoc(trail[i+1]))) {
                        loc = trail[i+1];
                    }
                    break;
                case DOUBLE_BACK_1 :
                    ...
                    break;
                case DOUBLE_BACK_2 :
                    if (!(isUnknownLoc(trail[i+2]))) {
                        loc = trail[i+2];
                    }
                    break;
                case DOUBLE_BACK_3 :
                    if (!(isUnknownLoc(trail[i+3]))) {
                        loc = trail[i+3];
                    }
                    break;
                case DOUBLE_BACK_4 :
                    if (!(isUnknownLoc(trail[i+4]))) {
                        loc = trail[i+4];
                    }
                    break;
                case DOUBLE_BACK_5 :
                    if (!(isUnknownLoc(trail[i+5]))) {
                        loc = trail[i+5];
                    }
                    break;
                case TELEPORT :
...
{
    LocationID currLoc = getLocation(dv->gv, PLAYER_DRACULA);
