...
{
   ht->nslots *= 2;
   List *new = malloc(ht->nslots * sizeof(struct _list_rep));
   for(int i=0; i<(ht->nslots/2;i++){
        int count = 0;
        int *array= valuesFromList(ht->chains[i], count);
        for(int j =0; j < count;j++) {
            int index = hash(array[j],ht->nslots);
            appendList(new[i], array[j]);
        }
   }
   ht->chains = new;
   return; 
